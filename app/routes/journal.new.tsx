...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (!content || content.trim().length === 0) {
    return Response.json({ error: "内容を入力してください。" } as ActionData, {
      status: 400,
    });
  }

  if (!mood || !validateMood(mood)) {
    return Response.json({ error: "気分を選択してください。" } as ActionData, {
      status: 400,
    });
  }
  ...
  try {
    ...
    if (error) {
      ...
      return Response.json(
        { error: "ジャーナルの作成に失敗しました。" } as ActionData,
        { status: 500 }
      );
    }
    ...
    return Response.json(
      { error: "予期しないエラーが発生しました。" } as ActionData,
      { status: 500 }
    );
  }
}
...
export default function JournalNew() {
  ...
  const handleSave = useCallback(
    async (content: string, mood: string, manualTags: string[]) => {
      setSaving(true);
