...
test.describe("Auth State Debug", () => {
  test("debug auth state and header visibility", async ({ page }) => {
    ...
    page.on('console', msg => {
      const text = msg.text();
      if (text.includes('[Root]') || text.includes('Header') || text.includes('BottomNav')) {
    ...
    const hasBottomNav = html.includes('ホーム') && html.includes('ノート') && html.includes('タグ');
    const hasGreeting = html.includes('こんばんは') || html.includes('こんにちは') || html.includes('おはようございます');
    ...
    const bottomNavVisible = await page.locator('div.fixed.bottom-0').isVisible().catch(() => false);
