...
async function testJWTDecoding() {
  ...
  try {
    ...
    console.log(`   📅 テストトークン発行時刻: ${new Date(payload.iat * 1000).toLocaleString()}`);
    console.log(`   📅 テストトークン期限: ${new Date(payload.exp * 1000).toLocaleString()}`);
...
async function testSessionStorage() {
  ...
  console.log(`      - MaxAge: ${sessionConfig.maxAge}秒 (${sessionConfig.maxAge / 86400}日)`);
...
async function testSupabaseAuthHelpers() {
  ...
  try {
    ...
    if (!session && !error) {
      ...
      console.log(`      期限: ${new Date(session.expires_at * 1000).toLocaleString()}`);
    } else {
